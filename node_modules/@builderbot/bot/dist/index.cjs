...
var require$$2$2 = require('querystring');
...
class BlackList {
    ...
    add(phoneNumbers) {
        ...
        numbersToAdd.flat().forEach((number) => {
...
class Queue {
    ...
    async enqueue(from, promiseInFunc, fingerIdRef) {
        ...
        const promiseFunc = (item) => {
            ...
            const timer = ({ reject }) => setTimeout(() => {
                reject(new Error(`Queue item timeout after ${this.timeout}ms`));
            }, this.timeout);
            const timerPromise = new Promise((resolve, reject) => {
                ...
                const refIdTimeOut = timer({ reject});
...
const generalDownload = async (url, pathToSave, headers) => {
    const checkIsLocal = fs.existsSync(url);
    const downloadDir = pathToSave ?? require$$0$4.tmpdir();
...
const logger = new console$1.Console({
    stdout: fs.createWriteStream(`${process.cwd()}/core.class.log`),
});
const loggerQueue = new console$1.Console({
    stdout: fs.createWriteStream(`${process.cwd()}/queue.class.log`),
});
        ...
        this.listenerBusEvents = () => [
            ...
            },
            {
                ...
                func: ({ instructions, title = '' }) => {
                    if (this.generalArgs.logs?.notices === false)
                        return;
...
class TestProvider extends ProviderClass {
    ...
    async delaySendMessage(milliseconds, eventName, payload) {
