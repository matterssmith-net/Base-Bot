	...
	var n = t("node:events").EventEmitter, r = t("node:child_process"), i = t("node:path"), a = t("node:fs"), o = t("node:process"), { Argument: s, humanReadableArgName: c } = ne(), { CommanderError: l } = te(), { Help: u, stripColor: d } = re(), { Option: f, DualOptions: p } = ie(), { suggestSimilar: m } = ae(), h = class e extends n {
		constructor(e) {
			super(), this.commands = [], this.options = [], this.parent = null, this._allowUnknownOption = !1, this._allowExcessArguments = !1, this.registeredArguments = [], this._args = this.registeredArguments, this.args = [], this.rawArgs = [], this.processedArgs = [], this._scriptPath = null, this._name = e || "", this._optionValues = {}, this._optionValueSources = {}, this._storeOptionsAsProperties = !1, this._actionHandler = null, this._executableHandler = !1, this._executableFile = null, this._executableDir = null, this._defaultCommandName = null, this._exitCallback = null, this._aliases = [], this._combineFlagAndOptionalValue = !0, this._description = "", this._summary = "", this._argsDescription = void 0, this._enablePositionalOptions = !1, this._passThroughOptions = !1, this._lifeCycleHooks = {}, this._showHelpAfterError = !1, this._showSuggestionAfterError = !0, this._savedState = null, this._outputConfiguration = {
				...
				writeErr: (e) => o.stderr.write(e),
				outputError: (e, t) => t(e),
				getOutHelpWidth: () => o.stdout.isTTY ? o.stdout.columns : void 0,
				getErrHelpWidth: () => o.stderr.isTTY ? o.stderr.columns : void 0,
				getOutHasColors: () => _() ?? (o.stdout.isTTY && o.stdout.hasColors?.()),
				getErrHasColors: () => _() ?? (o.stderr.isTTY && o.stderr.hasColors?.()),
				stripColor: (e) => d(e)
...
Expecting one of '${n.join("', '")}'`);
		...
		_exit(e, t, n) {
			this._exitCallback && this._exitCallback(new l(e, t, n)), o.exit(e);
		}
		action(e) {
			let t = (t) => {
...
-  already used by option '${t.flags}'`);
		...
		_optionEx(e, t, n, r, i) {
			if (typeof t == "object" && t instanceof f) throw Error("To add an Option object use addOption() instead of option() or requiredOption()");
		...
		_prepareUserArgs(e, t) {
			...
			if (t ||= {}, e === void 0 && t.from === void 0) {
				o.versions?.electron && (t.from = "electron");
				let e = o.execArgv ?? [];
				(e.includes("-e") || e.includes("--eval") || e.includes("-p") || e.includes("--print")) && (t.from = "eval");
			}
			e === void 0 && (e = o.argv), this.rawArgs = e.slice();
			let n;
			switch (t.from) {
				case void 0:
				case "node":
					...
					break;
				case "electron":
					o.defaultApp ? (this._scriptPath = e[1], n = e.slice(2)) : n = e.slice(1);
					break;
				case "user":
					n = e.slice(0);
					break;
 ...
 - ${t ? `searched for local subcommand relative to directory '${t}'` : "no directory for search for local subcommand, use .executableDir() to supply a custom directory"}`;
		...
		_executeSubCommand(e, t) {
			...
			function c(e, t) {
				...
				if (s.includes(i.extname(t))) return;
				let r = s.find((e) => a.existsSync(`${n}${e}`));
				if (r) return `${n}${r}`;
			}
			...
			if (this._scriptPath) {
				let e;
				try {
					e = a.realpathSync(this._scriptPath);
				} catch {
					e = this._scriptPath;
				}
				d = i.resolve(i.dirname(e), d);
			}
			if (d) {
				let t = c(d, u);
				if (!t && !e._executableFile && this._scriptPath) {
					let n = i.basename(this._scriptPath, i.extname(this._scriptPath));
					n !== this._name && (t = c(d, `${n}-${e._name}`));
				}
				u = t || u;
			}
			n = s.includes(i.extname(u));
			let f;
			o.platform === "win32" ? (this._checkForMissingExecutable(u, d, e._name), t.unshift(u), t = g(o.execArgv).concat(t), f = r.spawn(o.execPath, t, { stdio: "inherit" })) : n ? (t.unshift(u), t = g(o.execArgv).concat(t), f = r.spawn(o.argv[0], t, { stdio: "inherit" })) : f = r.spawn(u, t, { stdio: "inherit" }), f.killed || [
				"SIGUSR1",
				"SIGUSR2",
				"SIGTERM",
				"SIGINT",
			...
			f.on("close", (e) => {
				e ??= 1, p ? p(new l(e, "commander.executeSubCommandAsync", "(close)")) : o.exit(e);
			}), f.on("error", (t) => {
				if (t.code === "ENOENT") this._checkForMissingExecutable(u, d, e._name);
				else if (t.code === "EACCES") throw Error(`'${u}' not executable`);
				if (!p) o.exit(1);
				else {
					let e = new l(1, "commander.executeSubCommandAsync", "(error)");
					e.nestedError = t, p(e);
				}
		...
		_parseOptionsEnv() {
			this.options.forEach((e) => {
				if (e.envVar && e.envVar in o.env) {
					let t = e.attributeName();
					(this.getOptionValue(t) === void 0 || [
						"default",
						"config",
						"env"
					].includes(this.getOptionValueSource(t))) && (e.required || e.optional ? this.emit(`optionEnv:${e.name()}`, o.env[e.envVar]) : this.emit(`optionEnv:${e.name()}`));
				}
			});
		}
		_parseOptionsImplied() {
			let e = new p(this.options), t = (e) => this.getOptionValue(e) !== void 0 && !["default", "implied"].includes(this.getOptionValueSource(e));
		...
		usage(e) {
			if (e === void 0) {
				...
				let e = this.registeredArguments.map((e) => c(e));
		...
		nameFromFilename(e) {
			return this._name = i.basename(e, i.extname(e)), this;
		}
		executableDir(e) {
...
Expecting one of '${n.join("', '")}'`);
	...
	function _() {
		if (o.env.NO_COLOR || o.env.FORCE_COLOR === "0" || o.env.FORCE_COLOR === "false") return !1;
	...
	function p(e) {
		i(e), e.gracefulify = p, e.createReadStream = T, e.createWriteStream = E;
		var t = e.readFile;
		e.readFile = n;
		function n(e, n, r) {
...
function Dt(e, t, n) {
	return Object.hasOwn(t, n) && t[n] === void 0 ? [] : At(e[n] ?? [], t[n] ?? []);
}
var Ot = (e = {}, t = {}) => ({
	beforeRequest: Dt(e, t, "beforeRequest"),
	beforeRetry: Dt(e, t, "beforeRetry"),
	afterResponse: Dt(e, t, "afterResponse"),
	beforeError: Dt(e, t, "beforeError")
}), kt = (e, t) => {
	let n = new URLSearchParams();
	for (let r of [e, t]) if (r !== void 0) if (r instanceof URLSearchParams) for (let [e, t] of r.entries()) n.append(e, t);
...
var Vt = class e {
	...
	constructor(t, n = {}) {
		if (this.#r = t, this.#i = {
			...n,
			headers: Et(this.#r.headers, n.headers),
			hooks: Ot({
				beforeRequest: [],
				beforeRetry: [],
				beforeError: [],
			...
			method: jt(n.method ?? this.#r.method ?? "GET"),
	...
	async #l(e) {
		...
		if (Bt(e) && !this.#i.retry.retryOnTimeout) throw e;
		if (zt(e)) {
	...
	async #m() {
		...
		let e = Lt(this.request, this.#i);
		return this.#a = this.request, this.request = this.#a.clone(), this.#i.timeout === !1 ? this.#i.fetch(this.#a, e) : Ft(this.#a, e, this.#t, this.#i);
	}
	#h() {
		if (!this.#s) {
	...
	t.exports = function(e, t) {
		t ||= {};
		for (var o, s, c = !1, l = !1, u = 0, d = "", f = t.whitespace === !1 ? i : a, p = 0; p < e.length; p++) if (o = e[p], s = e[p + 1], !l && o === "\"" && (e[p - 1] === "\\" && e[p - 2] !== "\\" || (c = !c)), !c) {
			if (!l && o + s === "//") d += e.slice(u, p), u = p, l = n, p++;
			else if (l === n && o + s === "\r\n") {
				p++, l = !1, d += f(e, u, p), u = p;
				continue;
			} else if (l === n && o === "\n") l = !1, d += f(e, u, p), u = p;
			else if (!l && o + s === "/*") {
				d += e.slice(u, p), u = p, l = r, p++;
				continue;
			} else if (l === r && o + s === "*/") {
				p++, l = !1, d += f(e, u, p + 1), u = p + 1;
				continue;
			}
		}
		return d + (l ? f(e.substr(u)) : e.substr(u));
	};
})), Kt = /* @__PURE__ */ u(((e) => {
	var n = t("fs"), r = Wt(), i = t("path"), a = Gt(), o = e.parse = function(e) {
	...
	function r(e) {
		...
		else if (e instanceof RegExp) return new RegExp(e);
		else throw Error("Unexpected situation");
	}
	function i(e) {
		var t = [];
		return e.forEach(function(e, a) {
			typeof e == "object" && e ? Array.isArray(e) ? t[a] = i(e) : n(e) ? t[a] = r(e) : t[a] = o({}, e) : t[a] = e;
		}), t;
	}
	function a(e, t) {
	...
	var o = t.exports = function() {
		...
		return t.forEach(function(t) {
			typeof t != "object" || !t || Array.isArray(t) || Object.keys(t).forEach(function(l) {
				if (c = a(e, l), s = a(t, l), s !== e) if (typeof s != "object" || !s) {
					e[l] = s;
					return;
					...
					e[l] = r(s);
					return;
				} else if (typeof c != "object" || !c || Array.isArray(c)) {
					e[l] = o({}, s);
					return;
				} else {
					e[l] = o(c, s);
...
})), Yt = /* @__PURE__ */ y((/* @__PURE__ */ u(((e, n) => {
	var r = Kt(), i = t("path").join, a = qt(), o = "/etc", s = process.platform === "win32", c = s ? process.env.USERPROFILE : process.env.HOME;
	n.exports = function(e, t, n, l) {
		if (typeof e != "string") throw Error("rc(name): name *must* be string");
		n ||= Jt()(process.argv.slice(2)), t = (typeof t == "string" ? r.json(t) : t) || {}, l ||= r.parse;
...
function Xt(e) {
	let t = (0, Yt.default)("npm", { registry: "https://registry.npmjs.org/" }), n = t[`${e}:registry`] || t.config_registry || t.registry;
	return n.slice(-1) === "/" ? n : `${n}/`;
}
//#endregion
//#region node_modules/@pnpm/network.ca-file/node_modules/graceful-fs/polyfills.js
var Zt = /* @__PURE__ */ u(((e, n) => {
	var r = t("constants"), i = process.cwd, a = null, o = process.env.GRACEFUL_FS_PLATFORM || process.platform;
	process.cwd = function() {
		return a ||= i.call(process), a;
	};
	...
	var r = t("fs"), i = Zt(), a = Qt(), o = $t(), s = t("util"), c, l;
	/* istanbul ignore else - node 0.x polyfill */
	typeof Symbol == "function" && typeof Symbol.for == "function" ? (c = Symbol.for("graceful-fs.queue"), l = Symbol.for("graceful-fs.previous")) : (c = "___graceful-fs.queue", l = "___graceful-fs.previous");
	function u() {}
	function d(e, t) {
	...
	var r = rn(), i = t("path"), a = t("fs"), o = an(), s = t("events").EventEmitter, c = t("url"), l = t("http"), e = n.exports = function() {
	...
	var u = e.parse = function(e, t, n) {
		...
		else if (n === "json") if (this.emit) try {
			return JSON.parse(e);
		} catch (e) {
			this.emit("error", e);
		}
	...
	var n = t("fs"), r = t("path"), { envReplace: i } = ln();
	...
	var { readCAFileSync: r } = nn(), i = t("fs"), a = t("path"), { ConfigChain: o } = on(), s = sn(), c = un(), l = class extends o {
		constructor(e, t) {
			super(e), this.root = e, this._parseField = c.parseField.bind(null, t || dn());
		}
		add(e, t) {
			try {
...
})), hn = /* @__PURE__ */ y((/* @__PURE__ */ u(((e, t) => {
	var n = mn(), r = ":_authToken", i = ":_auth", a = ":username", o = ":_password";
	t.exports = function() {
		let e, t;
...
async function vn(e, t = {}) {
	let { version: n = "latest" } = t, { omitDeprecated: r = !0 } = t, i = e.split("/")[0], a = t.registryUrl ?? Xt(i), o = new URL(encodeURIComponent(e).replace(/^%40/, "@"), a), s = (0, hn.default)(a.toString(), { recursive: !0 }), c = { accept: "application/vnd.npm.install-v1+json; q=1.0, application/json; q=0.8, */*" };
	...
	try {
		...
		throw t?.response?.status === 404 ? new gn(e) : t;
	}
...
async function yn(e, t) {
	let { version: n } = await vn(e.toLowerCase(), t);
	return n;
}
//#endregion
...
function Nn({ onlyFirst: e = !1 } = {}) {
	return RegExp("(?:\\u001B\\][\\s\\S]*?(?:\\u0007|\\u001B\\u005C|\\u009C))|[\\u001B\\u009B][[\\]()#;?]*(?:\\d{1,4}(?:[;:]\\d{0,4})*)?[\\dA-PR-TZcf-nq-uy=><~]", e ? void 0 : "g");
}
//#endregion
...
function Hr() {
	...
	return Object.defineProperty(Y, "codes", {
		...
		hexToAnsi256: {
			value: (e) => Y.rgbToAnsi256(...Y.hexToRgb(e)),
			enumerable: !1
		},
		ansi256ToAnsi: {
			value(e) {
		...
		rgbToAnsi: {
			value: (e, t, n) => Y.ansi256ToAnsi(Y.rgbToAnsi256(e, t, n)),
			enumerable: !1
		},
		hexToAnsi: {
			value: (e) => Y.ansi256ToAnsi(Y.hexToAnsi256(e)),
			enumerable: !1
		}
	}), Y;
}
...
var X = "\n", Z = " ", Q = "none", ii = () => {
	...
	switch (n) {
		...
		case "right":
			...
			break;
		default:
			t = t.slice(i), t.length % 2 == 1 ? (t = t.slice(Math.floor(t.length / 2)), r = t.slice(1) + e + t) : (t = t.slice(t.length / 2), r = t + e + t);
			break;
	}
	return r;
	...
	r === "center" && o < s ? a = a.map((e) => Z.repeat((s - o) / 2) + e) : r === "right" && o < s && (a = a.map((e) => Z.repeat(s - o) + e));
	let c = Z.repeat(t.left), l = Z.repeat(t.right);
	return a = a.map((e) => {
		let t = c + e + l;
	...
	if (n.float === "center") {
		...
		let e = Math.max(s - t - n.margin.right - $(n.borderStyle), 0);
		c = Z.repeat(e);
	}
	let l = "";
...
var wi = T.dirname(E(import.meta.url)), Ti = 1e3 * 60 * 60 * 24, Ei = class {
	...
	async fetchInfo() {
		let { distTag: e } = this.#e, t = await yn(this._packageName, { version: e });
		return {
			latest: t,
			current: this.#t,
	...
	notify(e) {
		...
		let n = e.isGlobal ? `npm i -g ${this._packageName}` : `npm i ${this._packageName}`, i = "Update available " + r.dim("{currentVersion}") + r.reset(" → ") + r.green("{latestVersion}") + " \nRun " + r.cyan("{updateCommand}") + " to update", a = e.message || i;
		e.boxenOptions ??= {
			padding: 1,
			margin: 1,
...
(async () => {
	...
	if (t.update && t.update.latest !== p.version) {
		...
		t.notify({
			defer: !1,
			isGlobal: !0,
			message: `Update available ${e.dim("{currentVersion}")}${e.reset(" → ")}${t.update.type === "major" ? e.red("{latestVersion}") : t.update.type === "minor" ? e.yellow("{latestVersion}") : e.green("{latestVersion}")}
Run ${e.cyan("{updateCommand}")} to update
${e.dim.underline(t.update.type === "major" ? i.map((t) => e.dim.underline(t)).join("\n") : a)}`
		});
	}
	let r = process.argv.slice(2), i = r.findIndex((e) => e === "--help" || e === "-h");
	if (i !== -1 && r[i + 1]) {
		let e = r[i + 1].replace(/^-*/, "");
	...
	}) : null, C = (S?.args || []).filter((e, t, n) => (typeof e != "string" || !e.startsWith("-") || !l.includes(e)) && (typeof n[t - 1] != "string" || !n[t - 1].startsWith("-") || !l.includes(n[t - 1]))), w = [
